...
namespace Blynk.Virtual
{
    public class Client : IDisposable
    {
        ...
        public Action<int, double[]> OnVirtualMultiWritePin { get; set; }

        public Action<int, double> OnWriteVirtualPin { get; set; }
                                ...
                                if (messageLength <= offset)
                                {
                                    ...
                                    if (pinType == "vw")
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            {
                                                var pinValue = double.Parse(split[i], NumberStyles.Float, CultureInfo.InvariantCulture);
                                                values[i - 2] = pinValue;
                                            }
                                            this.OnVirtualMultiWritePin?.Invoke(pinId, values);
                                        }
